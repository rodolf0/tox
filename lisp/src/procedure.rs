...
pub type Fp = Rc<dyn Fn(&Vec<Expr>) -> Result<Expr, EvalErr>>;
...
impl cmp::PartialOrd for Procedure {
    fn partial_cmp(&self, _other: &Procedure) -> Option<cmp::Ordering> {
        None
    }
}
...
impl Procedure {
    pub fn new(params: Vec<String>, body: Expr, env: Rc<LispContext>) -> Procedure {
        Procedure {
            params: params,
            body: Body::Lisp(body),
            env: Some(env),
