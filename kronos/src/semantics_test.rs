...
fn test_seq_weekday() {
    ...
    assert_eq!(sunday.next().unwrap(),
               Range{start: dt(2016, 9, 4), end: dt(2016, 9, 5),
                     grain: Grain::Day});
    let mut sunday = Seq::weekday_back(0, false)(dt(2016, 8, 27));
    assert_eq!(sunday.next().unwrap(),
...
fn test_seq_month() {
    ...
    let mut weeks = Seq::month_back(2, false)(dt(2017, 7, 6));
    ...
    assert_eq!(weeks.next().unwrap(),
               Range{start: dt(2016, 2, 1), end: dt(2016, 3, 1),
                     grain: Grain::Month});
    let mut weeks = Seq::month_back(2, false)(dt(2017, 2, 6));
    assert_eq!(weeks.next().unwrap(),
               Range{start: dt(2016, 2, 1), end: dt(2016, 3, 1),
                     grain: Grain::Month});
}
...
fn test_seq_weekend() {
    ...
    let mut weekend = Seq::weekend_back(false)(dt(2016, 3, 20));
    assert_eq!(weekend.next().unwrap(),
               Range{start: dt(2016, 3, 12), end: dt(2016, 3, 14),
                     grain: Grain::Day});
}
...
#[test]
fn test_seq_grain() {
    let mut days = Seq::grain(Grain::Day)(dt(2015, 2, 27));
    ...
    let mut weeks = Seq::grain(Grain::Week)(dt(2016, 1, 1));
    assert_eq!(weeks.next().unwrap(),
    ...
    let mut months = Seq::grain(Grain::Month)(dt(2015, 2, 27));
    ...
    let mut quarters = Seq::grain(Grain::Quarter)(dt(2015, 2, 27));
    ...
    let mut years = Seq::grain(Grain::Year)(dt(2015, 2, 27));
    ...
    let mut minute = Seq::grain(Grain::Minute)(dt(2015, 2, 27));
    ...
    // backward iteration
    let mut years = Seq::grain_back(Grain::Year, false)(dt(2015, 2, 27));
    assert_eq!(years.next().unwrap(),
    ...
    let mut weeks = Seq::grain_back(Grain::Week, false)(dt(2017, 7, 6));
    assert_eq!(weeks.next().unwrap(),
    ...
    let mut minute =
        Seq::grain_back(Grain::Minute, false)(dttm(2015, 2, 27, 23, 20, 0));
    assert_eq!(minute.next().unwrap(),
               Range{start: dttm(2015, 2, 27, 23, 19, 0),
                     end: dttm(2015, 2, 27, 23, 20, 0),
                     grain: Grain::Minute});
    let mut minute =
        Seq::grain_back(Grain::Minute, false)(dttm(2015, 2, 27, 23, 20, 25));
    assert_eq!(minute.next().unwrap(),
               Range{start: dttm(2015, 2, 27, 23, 19, 0),
                     end: dttm(2015, 2, 27, 23, 20, 0),
                     grain: Grain::Minute});
    let mut minute =
        Seq::grain_back(Grain::Minute, false)(dt(2015, 2, 27));
    assert_eq!(minute.next().unwrap(),
...
fn test_nth_basic() {
    ...
    let jun28th =
        Seq::nthof(28, Seq::grain(Grain::Day), Seq::month_back(6, true));
    let mut jun28th = jun28th(dt(2016, 2, 25));
    assert_eq!(jun28th.next().unwrap(),
    ...
    let thirdwkjune =
        Seq::nthof(3, Seq::grain(Grain::Week), Seq::month_back(6, true));
    let mut thirdwkjune = thirdwkjune(dt(2016, 9, 4));
    assert_eq!(thirdwkjune.next().unwrap(),
    ...
    let tue3mo = Seq::nthof(
        3, Seq::weekday(2), Seq::grain_back(Grain::Month, true));
    let mut tue3mo = tue3mo(dt(2016, 2, 10));
    assert_eq!(tue3mo.next().unwrap(),
    ...
    let secmonapr = Seq::nthof(2, Seq::weekday(1), Seq::month_back(4, true));
    let mut secmonapr = secmonapr(dt(2016, 2, 25));
    assert_eq!(secmonapr.next().unwrap(),
    ...
    let thirdhour =
        Seq::nthof(3, Seq::grain(Grain::Hour), Seq::weekend_back(true));
    let mut thirdhour = thirdhour(dt(2016, 3, 20));
...
fn test_nth_discontinuous() {
    ...
    let feb29th = Seq::nthof(
        29, Seq::grain(Grain::Day), Seq::month_back(2, true));
    let mut feb29th = feb29th(dt(2015, 2, 25));
    assert_eq!(feb29th.next().unwrap(),
...
fn test_nth_composed() {
    ...
    let mo10th = Seq::nthof(
        10, Seq::grain(Grain::Day), Seq::grain(Grain::Month));
    let y5th10thday = Seq::nthof(5, mo10th, Seq::grain_back(Grain::Year, true));
    let mut y5th10thday = y5th10thday(dt(2015, 3, 11));
    assert_eq!(y5th10thday.next().unwrap(),
...
fn test_lastof() {
    ...
    let daybeforelastfeb = Seq::lastof(
        2, Seq::grain(Grain::Day), Seq::month_back(2, true));
    let mut daybeforelastfeb = daybeforelastfeb(dt(2015, 2, 25));
    assert_eq!(daybeforelastfeb.next().unwrap(),
    ...
    let daybeforelastfeb = Seq::lastof(
        5, Seq::grain(Grain::Day), Seq::month_back(2, true));
...
fn test_intersect() {
    ...
    let mon28th = Seq::intersect(Seq::weekday_back(1, false), Seq::nthof(
        28, Seq::grain(Grain::Day), Seq::grain_back(Grain::Month, true)));
    let mut mon28th = mon28th(dt(2016, 2, 25));
    assert_eq!(mon28th.next().unwrap(),
    ...
    let tue3pm = Seq::intersect(Seq::weekday(2), Seq::nthof(
        16, Seq::grain(Grain::Hour), Seq::grain(Grain::Day)));
...
fn test_multi() {
    ...
    let monfeb28th3d = Seq::nthof(
        28, Seq::grain(Grain::Day), Seq::grain_back(Grain::Month, true));
    let monfeb28th3d = Seq::intersect(monfeb28th3d, Seq::weekday_back(1, false));
    let monfeb28th3d = Seq::intersect(monfeb28th3d, Seq::month_back(2, false));
    let monfeb28th3d = Seq::shift(monfeb28th3d, Grain::Day, 3);
    let mut a_monfeb28th3d = monfeb28th3d(dt(2021, 9, 5));
    ...
    let mut a_monfeb28th3d = monfeb28th3d(dt(2022, 3, 1));
    assert_eq!(a_monfeb28th3d.next().unwrap(),
               Range{start: dt(2022, 3, 3), end: dt(2022, 3, 4),
                     grain: Grain::Day});

    // backward: monday feb 28th
    let monfeb28th = Seq::nthof_frame(
        28, Seq::grain(Grain::Day), Seq::grain_back(Grain::Month, true));
    assert_eq!(monfeb28th(dttm(2022, 2, 28, 1, 0, 0)).next().unwrap(),
               Range{start: dt(2022, 2, 28), end: dt(2022, 3, 1),
                     grain: Grain::Day});
    let monfeb28th = Seq::intersect(monfeb28th, Seq::weekday_back(1, true));
    let monfeb28th = Seq::intersect(monfeb28th, Seq::month_back(2, true));
    let mut a_monfeb28th = monfeb28th(dttm(2022, 2, 28, 1, 0, 0));
    assert_eq!(a_monfeb28th.next().unwrap(),
               Range{start: dt(2022, 2, 28), end: dt(2022, 3, 1),
                     grain: Grain::Day});
}
...
fn test_next() {
    ...
    assert_eq!(Seq::grain(Grain::Month).next(reftime, 1),
